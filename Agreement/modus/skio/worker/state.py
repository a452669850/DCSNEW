...
class SkWorkerState(object):
    ...
    # def fqfind(self, name: str, **kwargs) -> FVar:
    #     if not self.database:
    #         raise exception.SkError(exception.VAR_NOT_FOUND, name)
    #     return fqfind_variable(name, **kwargs)
    ...
    # def fqread(self, name: str, *, remote: bool = True, **kwargs) -> T_Val:
    #     """
    #     Read Variable Value
    #     :param name: SigName
    #     :param remote: read from remote device if remote else local memory
    ...
    # def fqwrite(self, name, value, *, remote=True, **kwargs):
    #     """
    #     Write Variable Value
    #     :param name: SigName
    #     :param value: Value
...
# def fqfind_variable(name: str, **kwargs) -> FVar:
#     """
#     查找变量信息
#     :param name:
#     :param kwargs:
...
#
#     if name in _alias:
#         return fqfind_variable(_alias[name], **kwargs)
#
#     model: PointModel = PointModel.filter(PointModel.sig_name == name).first()
#
#     if not model:
...
#                 res = PointModel.filter((PointModel.sig_name.contains(tx_keyword)) & (PointModel.channel == 'M1'))
