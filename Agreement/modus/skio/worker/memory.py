...
from Agreement.modus.skio.define import ValType, T_Val
...
class MemCache(object):
    ...
    def setup(self, path: Path) -> None:
        with self.lock:
            ...
            self.__create_mmap()
            self.__create_index()
            # pub.sendMessage('SKIO.MemCache.SETUP', a1=path)
    ...
    def read(self, var) -> T_Val:
        ...
        if block_info.var_type == ValType.U8:
            ...
            op = 1 << block_info.bit
            pub.sendMessage('SKIO.MemCache.READ', 传入值=var, 传出值=1 if byte & op == op else 0)
            return 1 if byte & op == op else 0
        else:
            raise TypeError(block_info)

    def write(self, var, val: T_Val) -> T_Val:
        self.__create_index()
        ...
        with self.lock:
            if bi.var_type == ValType.U8:
                ...
                raise TypeError(bi)
        pub.sendMessage('SKIO.MemCache.WRITE', 传入值1=var, 传入值2=val, 传出值=val)
        return val
