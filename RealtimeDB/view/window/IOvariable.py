...
from RealtimeDB.define import ProtocolType
from RealtimeDB.model import VarModel, DevModel
from RealtimeDB.view.dialog.updataVar import updataWindow
from RealtimeDB.view.thread.varThread import mythread
from RealtimeDB.view.varCoercion import varCoercion
from RealtimeDB.view.utils.querymodel import variableModel
from RealtimeDB.RealTrend import MatplotlibWidget
from xps.searchEdit import SearchLineEdit



class IOvariableWindow(QWidget):
...
class tabWidget(QWidget):
    ...
    def showContextMenu(self):  # 创建右键菜单
        ...
        self.actionC = self.tableView.contextMenu.addAction('实时趋势')
        self.tableView.contextMenu.popup(QCursor.pos())  # 1菜单显示的位置
        self.actionC.triggered.connect(self.actionHandler2)
    ...
    def actionHandler2(self):
        row = self.tableView.currentIndex().row()
        var_name = self.queryModel.datas[row][1]
        win = MatplotlibWidget(name = var_name)
        win.show()
