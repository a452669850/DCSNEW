...
from communication.define import LOGGER, ProtocolType, T_Val, IVar, SigType, ValType, IDev
from communication.model import init_database, DevModel, VarModel
from communication.protocol.pms import SmPXIDev, ModBus
from communication.worker.memory import MemCache
from communication.worker.sample import SampleThread
...
class SkWorkerState(object):
    ...
    def setup(self, path: Path) -> 'SkWorkerState':
        ...
        # 4. Slot
        self.mem.setup(path)
        self.slots = list(map(register_dev, DevModel.select()))
        # sample thread
...
def register_dev(dev: DevModel) -> 'SlotInfo':
    """
...
def find_variable(name: str, **kwargs) -> IVar:
    ...
    model: VarModel = VarModel.filter(VarModel.sig_name == name).first()
    if not model:
        raise exception.SkError(exception.VAR_NOT_FOUND, name)
