...
from historyinfluxDB.historyDB import select_library

treelist = select_library()[1:]
...
class TreeDockWidget(QDockWidget):
    ...
    def onTreeClicked(self, index: QModelIndex):
        if index.parent().data() not in [self.parent, None]:
            for i in treelist:
                if i['name'] == index.data():
                    self.tree_Signal.emit(i['name'])
